...
class EnrouteRequest: FlightAwareRequest<FAFlight> ,
                      Codable {
    ...
    static func create(airport: String ,
                       howMany: Int? = nil)
        -> EnrouteRequest {
            
        if
            let request = requests[airport] {
    ...
    } // static func create(airport: ,howMany: ) -> EnrouteRequest {}
    
    
    override func decode(_ data: Data)
        -> Set<FAFlight> {
            
        let result = (try? JSONDecoder().decode(EnrouteRequest.self ,
                                                from : data))?.flightAwareResult
        offset = result?.next_offset ?? 0
            
        return Set(result?.enroute ?? [])
    } // override func decode() -> Set<FAFlight> {}
    
    
    override func filter(_ results: Set<FAFlight>)
        -> Set<FAFlight> {
            
        results.filter { $0.arrival > Date.currentFlightTime }
    } // override func filter() -> Set<FAFlight> {}
